...
- For candidates with `准入结果=PASS`, write three 1–5 scores: `停留潜力分` 35%, `证明与转化分` 40%, and `平台感与执行分` 25%. `综合评分` is a formula field and calculates only when admission is PASS and all three scores are present. Do not write legacy duplicate score or audit fields.
...
- **Automatic task rotation:** before ordinary original ideation, fresh-read the selected task's `任务状态`、`内容库版本列表`、`内容版本数（自动计数）` and `轮换上限`. The automatic count is derived from the duplex link and is the sole rotation count; the legacy numeric field `已生成版本数` is not incremented or used for decisions. Default `轮换上限` is 5 when blank; never silently lower an explicit limit. If the task is `已归档` or the automatic count is at/above the limit, stop using it. Create one new `原创` task with the same product, platform/account, priority, subject asset and long-term boundaries, but a materially different creative direction selected after reading recent content history; set `轮换来源任务` to the exhausted task, `任务状态=活跃`, and inherit `轮换上限`. Mark the exhausted task `已归档`, write `归档时间`, and preserve every old task/version link. If any write fails, stop and report the inconsistency instead of creating a second replacement task. Never delete a `内容库` version; set its `版本状态=已归档` and write `归档时间` when it leaves the active workflow.
...
- After every named, approved final-generation segment has succeeded, assemble the exact requested 20-, 30-, or 40-second video and technically verify its video stream, audio stream when requested, and duration. Fresh-read the source `内容库` record and confirm that the linked record exists before creating one `最终成片` record. Write `成片名称`, a unique `成片ID`, `内容版本`, `内容版本校验状态=有效`, `视频时长（秒）`, and `生成时间`, then attach only the complete publishable video to `最终视频`. If the source record cannot be resolved, create no publishable final-film record; preserve any existing attachment and mark the record `内容版本校验状态=需修复`. Do not create a final-film record for failed jobs, raw segments, retries, or unverified assemblies. For uploads up to 20 MB, use `scripts/upload_feishu_attachments.py`; above 20 MB, use a verified multipart upload or the Feishu UI, then verify the attachment token, filename, and playable file before reporting delivery.
