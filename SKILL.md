...
Read [references/video-prompt-contract.md](references/video-prompt-contract.md) before writing any board or video prompt. Every final-generation prompt must use its five-block structure and the input-role authority order; asset routing and prompt wording are one gate, not separate optional steps.
...
Source-file encoding must follow exactly one of these two paths:

- A normal `.png`, `.jpg`, `.jpeg`, or `.webp` file is binary image data. Read its raw bytes once, then Base64-encode those bytes once.
- A `.b64` sidecar emitted by `prepare_flow_inputs.py` is already Base64 text. Read the text, trim only surrounding whitespace/newlines, and use it directly as `data_base64`; never Base64-encode the sidecar file's text bytes again.

Never mix these paths. A `.b64` sidecar that is Base64-encoded a second time produces a syntactically valid outer payload whose decoded bytes are text rather than an image, and may be reported downstream as a misleading upload or project error.
...
2. For a downloaded normal image file, read the raw bytes, determine the actual MIME type, and Base64-encode the bytes exactly once. For a cached `.b64` sidecar, read the already-encoded text directly and retain the MIME type from the cache manifest or decoded image. Send only the required `mime_type` and `data_base64` fields in the Flow2API `input_images` payload. Do not send a `file_token` in place of `data_base64`; do not assume that a local path, Feishu URL, or `data:image/...;base64,` URI prefix is accepted unless the current MCP schema explicitly documents it.
3. Validate locally before submission: every item has a supported image MIME type, non-empty Base64, strict Base64 decoding succeeds, the decoded bytes have the expected image type, and the decoded image SHA-256 matches the source image SHA-256. For a `.b64` sidecar, this hash check must compare the decoded sidecar bytes with the cache manifest's source/rendered image hash; it is the required guard against double-encoding. Deduplicate identical files by stable SHA-256 hash before applying the R2I limit of 10 or the Omni R2V limit of 7. If a required asset cannot be serialized or the limit is exceeded, stop before submission and record the asset conflict.
...
- The helper emits local `.b64` sidecar paths rather than raw Base64 in the manifest. When building the in-memory MCP payload, read the sidecar text directly after trimming surrounding whitespace; do not Base64-encode the sidecar file again. Do not paste raw Base64 into prompts, package reports, or Feishu fields.
...
- Every prompt containing a recurring subject must include the `SUBJECT IDENTITY LOCK` block from [references/video-prompt-contract.md](references/video-prompt-contract.md), with the stable identity description repeated in every Segment. “Same dog/person” without the identity attributes is not a continuity lock.
...
Before drafting `画面生成 Prompt` or `视频生成 Prompt`, also read [references/video-prompt-contract.md](references/video-prompt-contract.md). Resolve the target spoken language from the task and approved script; do not silently translate, mix Chinese and Thai, or use the language of the agent's explanation as the language of the generated audio.
...
- Every configuration-derived board prompt and every Omni Segment prompt must contain the five mandatory blocks in [references/video-prompt-contract.md](references/video-prompt-contract.md): input roles/authority, product hard constraints, subject identity lock, language/audio/timed dialogue, and no-text/cross-Segment continuity. A prompt that omits a block or contains a task-language conflict is invalid and must not be submitted.
...
- Treat `内容库.已执行次数` as the count of completed, accepted final films—not submission attempts, segment Jobs, retries, failures, or credit usage. Before starting a named approved generation, fresh-read both `已执行次数` / `执行次数上限` and the `最终成片` records linked to that exact content version. Count only qualifying final-film records whose complete target-duration video has passed technical verification and product/content visual review and whose `最终视频` attachment has been fresh-read as present and playable. If the stored count and qualifying linked-film count disagree, stop and report the data inconsistency; do not guess, reserve, increment, archive, or submit a new generation. If the verified count is already at the limit, archive the content version and stop. Otherwise submit the authorized raw-segment Jobs without changing `已执行次数`; record every submission, retry, failure, timeout, and partial segment only in `generation-jobs.json`.
- After every required segment succeeds, assemble the configured target video, verify its streams and exact duration, and complete product/content visual review. Fresh-read the source `内容库` record before creating one `最终成片` record. Write `成片名称`, a unique `成片ID`, `内容版本`, `视频时长（秒）`, and `生成时间`, then attach only the complete accepted video to `最终视频`. Fresh-read the new final-film record and require the expected linked content version plus a present, playable attachment; only then increment the source content record's `已执行次数` by exactly one and re-read it. Archive the content version only when this post-verification count reaches the limit. A failed, cancelled, timed-out, unassembled, technically invalid, visually rejected, or partial run never increments the count and never triggers count-based archival. Each successful repeat final film creates one additional `最终成片` record and increments the count once. If any final-film create/upload/verification/count write fails, retain the artifacts and report the inconsistency; do not create a duplicate final-film record. For uploads up to 20 MB, use `scripts/upload_feishu_attachments.py`; above 20 MB, use a verified multipart upload or the Feishu UI, then verify the attachment token, filename, and playable file before reporting delivery. Do not create or read a separate execution-record table or an抽卡状态 field.
...
- Raw Omni generation must explicitly forbid captions, subtitles, dialogue transcription, labels and readable text in every language; the presence of spoken dialogue does not relax this rule. Burn subtitles only when explicitly requested as a separate post-assembly step, after saving ASR-aligned timing artifacts and the exact subtitle source. Do not put subtitle text into an Omni raw-segment prompt.
