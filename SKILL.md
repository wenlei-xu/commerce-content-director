...
- Only after every expected board exists, has exactly four chronological 9:16 panels, has a recorded validator `PASS`, and has passed per-panel interaction/identity/text review, create the new `内容库` record for a new version (or update the same rejected record). In that single post-validation archive step, write the final script, `画面生成 Prompt`, creative fields, content fingerprint, `Agent 自检`, selected product image, every final-generation board, and, for `全量复刻`, all final storyboard master attachments. Before writing `待审核`, fresh-read the record and verify that every expected board attachment is present.
- Upload all reusable product images into the relevant `产品` attachment fields; for replication, upload the reference video directly to `内容策划任务.爆款视频` or provide `内容策划任务.爆款视频链接`. In original mode, do not upload reference media to the task. Upload every Flow2API final-generation board to `最终分镜图` **only after the 9:16-panel validator has passed**. Use `scripts/upload_feishu_attachments.py` when the Feishu MCP exposes record CRUD but no media-upload tool. The script reads `FEISHU_APP_ID` and `FEISHU_APP_SECRET` from the workspace `.hermes/.env`, obtains a tenant token, chooses `bitable_image` for images and `bitable_file` for other media, then writes the returned attachment tokens to the named record field. The direct path supports files up to 20 MB; use Feishu UI or a verified multipart flow for larger media. Never print credentials or tokens.
- Example: `python scripts/upload_feishu_attachments.py --record-id <record_id> --field 最终分镜图 <Segment-01.png> <Segment-02.png>`. Do not substitute a local path, a cloud-document link, or a guessed token for an attachment upload.
...
- `scripts/upload_feishu_attachments.py --record-id <id> --field <field> <file...>`: uploads one or more local images to a Base attachment field using the workspace's existing Feishu app credentials.
