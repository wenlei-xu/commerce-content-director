...
description: "Run Feishu-driven ecommerce short-video production around a Markdown creation workbook: align approved voiceover with ASR-timed visual segments, generate reusable clips or final films, render final MP4 with FFmpeg or optional Remotion templates, and preserve resumable execution records."
---
...
Every active run uses the Skill-local `runs/<run_id>/` directory. Active scripts create or resume it through the `scripts/run_context.py` module; keep inputs, prompts, execution plans, generation outputs, subtitles and QA inside that run. Do not create new runtime artifacts directly under `plan/`, the current working directory, `scripts/` or `references/`.
...
- **Generate and accept a final video**: read [final-video.md](references/workflows/final-video.md); when automated animated subtitles, bubbles, product cards or other code-driven visual templates are requested, also read [remotion.md](references/rendering/remotion.md).
...
### Automated rendering backends

- Final-video output must be an automatically rendered MP4. Use the existing FFmpeg backend for concatenation, audio mixing, static subtitles, codec normalization and QA. Use the optional Remotion backend when the requested output needs reusable animated subtitles, dog inner-voice bubbles, monitoring overlays, product reveal cards or other Petopia visual templates; read [remotion.md](references/rendering/remotion.md) before doing so.
- Remotion is a local code-rendering backend, not Computer Use. Never control Jianying, simulate mouse/keyboard input, or require a Jianying export click in a final-video run. A Jianying draft may be produced as an optional review artifact but cannot be the required final deliverable.
- Record `render_backend=ffmpeg|remotion|hybrid` in the run manifest and render specification. Do not silently switch a selected Remotion or hybrid run to Jianying or another renderer. If the selected Remotion project or composition is unavailable, stop with the concrete dependency error.
- The existing `scripts/assemble_final_video.py` remains the default FFmpeg path. The local `scripts/render_remotion.py` helper may invoke only an already-installed local Remotion project with `npx --no-install`; it must not install dependencies or hide a failed render.
...
- [run-artifact-contract.md](references/invariants/run-artifact-contract.md) — Skill-local run directory and artifact write rules.
...
- [remotion.md](references/rendering/remotion.md) — optional automated Remotion rendering backend, render spec and no-Computer-Use boundary.
...
- 字幕运行产物统一写入当前运行的 `subtitles/`；字幕源文件、ASR 时间和 QA 证据必须能从 `run-manifest.json` 追溯。
- 字幕位置不是自由文本：生成字幕时同时写入 `subtitles/subtitle-layout.json`，并严格使用统一的 720x1280 抖音安全区契约——底部居中锚点，左/右边界 `x=28/692`，字幕框底边 `y=940`，两行字幕的上边界 `y=820`，中心约在从底部上移三分之一的位置，最多两行；底部预留 340px，避免落入底部文案、评论和互动控件区域。1080x1920 等同为 9:16 的输出按画布比例缩放这些坐标；不得在不同脚本、不同字幕格式或不同视频间临时改 `MarginV`、对齐方式或左右边界。
...
- **Subtitle styling:** Chinese 720x1280 burned-in subtitles use the fixed style `SimHei`, bold, `FontSize=50`, white text, `Outline=1px` black outline, `Shadow=0`, no black box/background, `Alignment=2`, `MarginL=28`, `MarginR=28`, and `MarginV=340`. These are fixed production parameters, not starting values, and must not be changed per video. Inspect a rendered QA frame at actual output resolution only to verify readability, deliberate one-line/two-line layout, safe-area placement and subject/product obstruction; if a cue is too wide, revise its semantic line break rather than changing the fixed style.
- **Mandatory subtitle self-review:** after subtitle burn-in, the agent must look back at QA frames at the actual output resolution and complete a visual self-review before accepting the video. Check, at minimum: (1) the text is large enough to read on a phone; (2) each cue has a deliberate one-line or two-line layout, with no accidental third line; (3) line breaks follow meaning and both lines are visually balanced; (4) the rendered text box stays inside the declared safe rectangle (`x=28..692`, `y=820..940` on 720x1280, scaled to the actual output); and (5) the subtitle does not cover the product or subject. If any item fails, mark the layout QA as failed, return to subtitle layout, re-render and review again. A subtitle track is not accepted merely because the SRT/ASS file parses successfully.
