...
Use the registered Flow2API MCP and never call private model endpoints directly. Storyboard-image Jobs may use the current catalog-selected image capability. Final-video Jobs have a fixed model contract: use the registered `omni_portrait` model only. Omni produces exactly one 10-second portrait 9:16 raw segment per Job; assemble 20/30/40-second deliverables from exactly 2/3/4 chronological segments. Use the current catalog only to verify that `omni_portrait` is available and to read its current R2V input limit. If it is unavailable, stop; never substitute Veo, a generic portrait model, or another provider. Image payloads must contain validated raw image bytes in the current MCP schema; never place tokens, URLs, audit objects, or Base64 in prompts or Feishu fields.

Treat an approved multi-segment script or video prompt as planning source material, never as a reusable Omni Job payload. Compile one segment-scoped prompt per 10-second Job: it may contain only that Segment's literal spoken lines, with timings rebased to the local `0.0–10.0s` window, and must not quote dialogue from earlier or later Segments anywhere in the prompt. Validate that approved dialogue is assigned exactly once across the complete Segment set unless the approved script explicitly marks an intentional repeat. If a line crosses a Segment boundary or cannot be assigned unambiguously, stop before submission and fix the approved timing data.
...
Create a minimal local run package before generation. At minimum retain `package.json`, `generation-jobs.json`, `dialogue-allocation.json`, `quality-report.md`, exact submitted per-segment prompts, asset role/hash mappings, and temporary outputs. The package is evidence, not a replacement for Feishu source records.
