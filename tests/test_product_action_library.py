...
class ProductActionLibraryTest(unittest.TestCase):
            ...
            "tables": {
                "product_action_library": {
                    ...
                    "fields": {
                        ...
                        "how_to": "怎么玩",
                        "visual_focus": "画面重点",
                        "action_reference_images": "动作示意图",
                        "related_benefit": "关联卖点（可选）",
                    },
                }
            },
        }
        self.records = [
            {"record_id": "action-1", "fields": {"产品": ["product-durian"], "动作": "拔河", "怎么玩": "轻拉互动", "画面重点": "接触点可见", "动作示意图": [{"file_token": "img-1", "name": "pull.png", "type": "image/png"}], "关联卖点（可选）": "连接牢固", "是否可用": "可用"}},
            {"record_id": "action-2", "fields": {"产品": ["product-durian"], "动作": "未确认动作", "怎么玩": "待核实", "画面重点": "待核实", "动作示意图": [], "关联卖点（可选）": "", "是否可用": "待确认"}},
            {"record_id": "action-3", "fields": {"产品": ["product-pineapple"], "动作": "推动", "怎么玩": "推动探索", "画面重点": "结构可见", "动作示意图": [], "关联卖点（可选）": "", "是否可用": "可用"}},
        ]
    ...
    def test_returns_optional_reference_data_without_making_it_required(self):
        result = self.module.query_actions(FakeFeishu(self.records), self.schema, "product-durian")
        action = result["actions"][0]
        self.assertEqual(action["action_reference_images"], [{"file_token": "img-1", "name": "pull.png", "type": "image/png"}])
        self.assertEqual(action["related_benefit"], "连接牢固")
