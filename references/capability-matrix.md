...
| `storyboard_generation` | required | required | not required | not required | not required | required |
...
An unavailable or unverified GPT Image 2 route is a stop condition for storyboard generation. Do not substitute Flow2API, another image model or another provider. Flow2API health affects final-video generation only.
