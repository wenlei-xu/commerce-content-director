...
Use this workflow before writing Beats for a pet-product short video. Its only output is one complete **动作导演说明** paragraph that gives the scene's action arc; it is planning text, not a new JSON file, table field, or approval object.
...
Describe only visible scene, movement, and reaction. Do not turn the paragraph into a product-feature explanation, sales claim, mechanism proof, or action inventory. Choose a main interaction that is different from the batch's already-used main interaction when product facts or user confirmation support it. If the available evidence supports only a narrow set of actions, stay within that boundary rather than inventing an action.
