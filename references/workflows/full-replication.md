...
- A `target_production_segment` is the generation unit. It is always the configured 10 seconds and produces one 9:16 first-frame image for each of the two A/B script versions (with local retry attempts when needed). The first frame represents the Segment's entering state at local `t=0`; it does not depict the full 10-second action. Human acceptance happens at the complete first-frame-version level, which contains all target production Segments in order.
...
4. Active configuration owns 10-second production segmentation and one 9:16 first frame per generated target production Segment/version pair.
...
4. Generate two self-contained first-frame packages for each target production Segment, one for script version A and one for B. The `Director` deep module owns the first frame's static moment, camera, composition, performance, continuity and variant delta. The generated first frame represents the target Segment's entering composition/state; in high-fidelity replication, the routed start frame owns source entering composition/state and the routed result frame remains evidence for the target script's payoff/handoff. The source visual-style profile owns the capture treatment across all first frames and all Segments. Under `preserve_source_subject`, the frames also preserve the original person or animal and no subject anchor is added. Under `replace_subject`, the selected subject anchor overrides source-subject identity. The current product anchor always overrides source-product identity. Keep generation artifacts in the run package; do not create a candidate or review table.
