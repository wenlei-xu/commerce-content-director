...
6. Inspect sampled frames and every subtitle-change boundary with the model's visual recognition, following [model-visual-text-recognition.md](../invariants/model-visual-text-recognition.md). Verify subtitle presence/timing when speech exists and verify no unintended text outside the subtitle layer. Do not call a local or third-party OCR engine. Repeat the product/subject/scene/audio review on the final candidate.
