...
4. A same-attempt transport retry reuses the same idempotency key and candidate identity. An intentional new visual candidate for the same Segment increments `attempt` and uses a new key. Persist the returned `job_id`, segment ID, attempt and idempotency key in the local run manifest before waiting. Multiple candidate Jobs do not increase the expected accepted-board count.
