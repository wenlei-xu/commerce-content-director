...
3. Run `scripts/validate_generation_storyboards.py` with the active configuration snapshot, then perform the workflow's visual, product-fidelity and script-coverage preflight. Treat this as machine/agent preflight evidence, not final visual approval. Write each passing complete A/B package directly to its script-version record; never publish four panels as separate approval items.
4. Count qualified boards per script version and Segment. Technical failures and invalid complete boards create only the missing A/B package slots. Submit all simultaneously known missing slots as one repair batch when there are two or more; use a single submit only for one missing slot. Never regenerate a successful version/Segment board merely because another batch item failed.
