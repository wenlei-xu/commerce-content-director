...
`Director` is the single seam between a locked script and storyboard prompt
...
For every target production Segment and each version, return exactly four ordered
Panel decisions. A Panel decision contains:
...
The output is a new local storyboard plan consumed by the prompt compiler. It
...
The only remote storyboard mutation is the complete A/B package written by
`scripts/publish_storyboard_versions.py` to two script records linked through
...
```json
{
  ...
  "versions": {
    "A": {"variant_delta": "...", "boards": [{"segment_id": "Segment-01", "path": "...", "sha256": "..."}]},
    "B": {"variant_delta": "...", "boards": [{"segment_id": "Segment-01", "path": "...", "sha256": "..."}]}
  }
}
```
