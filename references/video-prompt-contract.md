...
The video model is fixed by Segment duration: 4-second, 6-second, 8-second and 10-second Segments use only the exact confirmed Omni portrait model for that duration. Short Segments may occur anywhere in the final sequence; they are not tail-only. Arbitrary per-Segment model overrides and non-Omni video models are forbidden.

Every final-generation prompt uses a supported portrait reference-to-video model and an explicit segment duration from the duration plan. A Segment is a direct 4-second, 6-second, 8-second or 10-second portrait 9:16 Job using the exact matching Omni portrait model (for example, `gemini_omni_r2v_portrait_4s`, `gemini_omni_r2v_portrait_6s`, `gemini_omni_r2v_portrait_8s` or `omni_portrait`, only when confirmed by the live model catalog). Choose the duration from the ASR-timed visual action: short atomic shots may be 4–6s while continuous action arcs may be 8–10s. The model is determined by the Segment duration; arbitrary per-Segment model overrides and non-Omni video models are forbidden. Never submit a longer Job and crop it. The complete film is assembled from the exact chronological Segment durations in the duration plan, such as `4s + 6s + 4s`.
...
- Assign each approved spoken line to exactly one duration-plan Segment unless the approved script explicitly marks an intentional repeat. Every 4-second, 6-second, 8-second or 10-second Segment is a complete Segment with its own local timeline; a line may not cross a Segment boundary.
